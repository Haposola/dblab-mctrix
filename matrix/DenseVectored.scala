...
class DenseVectored(
                      private[mctrix] val rows: RDD[(Long, BDV[Double])],
                      private var nRows: Long,
                      private var nCols: Long) extends Distributed with Logging {
  ...
  override private[mctrix] def toBreeze(): BDM[Double] = {
    val m = numRows().toInt
    val n = numCols().toInt
  ...
  private[mctrix] def getClusterCores(): Int = {
    val sc = rows.context
   ...
   * @return
   */
  def multiply(other: Distributed, cores: Int): Distributed = {
    multiply(other, cores, 300)
  }


  // better transformation 
  def multiply(other: DenseVectored, splitMode: (Int, Int, Int)): Blocked = {
    ...
    new Blocked(result, numRows(), other.numCols(), m, n)
  }
   ...
   */
  def multiply(other: Distributed,
                     cores: Int,
                     broadcastThreshold: Int = 300): Distributed = {
    other match {
  ...
  def cBind(other: Distributed): Distributed = {
    ...
    other match {
      case that: DenseVectored =>
        ...
        new DenseVectored(result, numRows(), numCols() + that.numCols())
      case that: Blocked =>
        val thatDenVec = that.toDenseVecMatrix()
        cBind(thatDenVec)
      case that: Distributed =>
   ...
   */
  private[mctrix] def rowExchange(permutation: Array[Int]): DenseVectored = {
    ...
    new DenseVectored(result, numRows(), numCols())
  }
   ...
   */
def luDecompose(mode: String = "auto"): (Blocked, Array[Int]) = {
    ...
    val (luResult: Blocked, perm: Array[Int]) = computeMode match {
      case LUmode.LocalBreeze => {
        ...
        (new Blocked(blk, lu._1.rows, lu._1.cols, 1, 1), pArray)
      }

      case LUmode.DistSpark => 
        val subMatrixBaseSize = rows.context.getConf.getInt("mctrix.lu.basesize", 1000)
        ...
        val result = new Blocked(blkMat, numRows(), numCols(), numBlksByRow, numBlksByCol)
   ...
   */
  def choleskyDecompose(mode: String = "auto"): Blocked = {
    require(numRows() == numCols(),
      s"LU decompose only support square matrix: ${numRows()} v.s ${numCols()}")
    object LUmode extends Enumeration {
      val LocalBreeze, DistSpark = Value
    ...
    val (luResult: Blocked) = computeMode match {
      case LUmode.LocalBreeze =>
        val brz = toBreeze()
        val l = brzCholesky(brz)
        val blk = rows.context.parallelize(Seq((new BlockID(0, 0), l)), 1)
        (new Blocked(blk, l.rows, l.cols, 1, 1))
      case LUmode.DistSpark => {
        val subMatrixBaseSize = rows.context.getConf.getInt("mctrix.cholesky.basesize", 1000)
        val numBlksByRow, numBlksByCol = math.ceil(numRows().toDouble / subMatrixBaseSize.toDouble).toInt
        val subMatrixBase = math.ceil(numRows().toDouble / numBlksByRow.toDouble).toInt
   ...
   * @return
   */
  def inverse(mode: String = "auto"): (Blocked) = {
    require(numRows() == numCols(),
    ...
    val luResult: Blocked = computeMode match {
      case LUmode.LocalBreeze => {
        ...
        new Blocked(blk, inverse.rows, inverse.cols, 1, 1)
      }

      case LUmode.DistSpark => {
        val subMatrixBaseSize = rows.context.getConf.getInt("mctrix.inverse.basesize", 1000)
        val numBlksByRow, numBlksByCol = math.ceil(numRows().toDouble / subMatrixBaseSize.toDouble).toInt
        val subMatrixBase = math.ceil(numRows().toDouble / numBlksByRow.toDouble).toInt
        val pArray = Array.ofDim[Int](numRows().toInt)
        ...
        new Blocked(blkMat, numRows(), numCols(), numBlksByRow, numBlksByCol)
      }
    }
    luResult
  }
   ...
   * @param other another matrix in DenseVecMatrix format
   */
  final def add(other: Distributed): DenseVectored = {
    other match {
      case that: DenseVectored =>
   ...
   * @param other another matrix in DenseVecMatrix format
   */
  final def subtract(other: Distributed): DenseVectored = {
    other match {
      case that: DenseVectored =>
   ...
   */
  final def add(b: Double): DenseVectored = {
   ...
   */
  final def subtract(b: Double): DenseVectored = {
    val result = rows.mapPartitions(iter => {
      iter.map(t => (t._1, BDV(t._2.toArray.map(_ - b))))
    }, true)
    new DenseVectored(result, numRows(), numCols())
   ...
   */
  final def subtractBy(b: Double): DenseVectored = {
    val result = rows.mapPartitions(iter => {
   ...
   * @param b a number in the format of double
   */
  final def multiply(b: Double): DenseVectored = {
    val result = rows.mapPartitions(iter => {
      iter.map(t => (t._1, BDV(t._2.toArray.map(_ * b))))
    }, true)
    new DenseVectored(result, numRows(), numCols())
   ...
   */
  final def divide(b: Double): DenseVectored = {
    val result = rows.mapPartitions(iter => {
      iter.map(t => (t._1, BDV(t._2.toArray.map(_ / b))))
    }, true)
    new DenseVectored(result, numRows(), numCols())
   ...
   * @param b a number in the format of double
   */
  final def divideBy(b: Double): DenseVectored = {
    val result = rows.mapPartitions(iter => {
      iter.map(t => (t._1, BDV(t._2.toArray.map(b / _))))
   ...
   */
  final def sliceByRow(startRow: Long, endRow: Long): DenseVectored = {
    require((startRow >= 0 && endRow <= numRows() && startRow <= endRow),
      s"start row or end row mismatch the matrix num of rows")

    new DenseVectored(rows.filter(t => (t._1 >= startRow && t._1 <= endRow)).map(t => (t._1 - startRow, t._2)), endRow - startRow + 1, numCols())
  }
   ...
   */
  final def sliceByColumn(startCol: Int, endCol: Int): DenseVectored = {
    require((startCol >= 0 && endCol <= numCols() && startCol <= endCol),
      s"start column or end column mismatch the matrix num of columns")

    new DenseVectored(rows.map(t => (t._1, BDV(t._2.toArray.slice(startCol, endCol + 1)))), numRows(), endCol - startCol + 1)
  }
   ...
   * @param endCol the end column of the subMatrix, this column is included
   */
  final def getSubMatrix(startRow: Long, endRow: Long, startCol: Int, endCol: Int): DenseVectored = {
  ...
  def toBlockMatrix(splitStatusByRow: Array[ArrayBuffer[(Int, (Int, Int), (Int, Int))]],
    ...
    new Blocked(blocks, numRows(), numCols(), blksByRow, 1)
  }

  def toBlockMatrix(numByRow: Int, numByCol: Int): Blocked = {
    val mRows = numRows().toInt
    val mColumns = numCols().toInt
    ...
    if (blksByCol == 1) {
      ...
      new Blocked(result, numRows(), numCols(), blksByRow, blksByCol)
    } else {
      val result = rows.mapPartitions(iter => {
        iter.flatMap { case (index, vector) =>
          var startColumn = 0
   ...
   */
  def toSparseVecMatrix(): SparseVectored = {
    val result = rows.mapPartitions(iter => {
      iter.map(t => {
    ...
    new SparseVectored(result, numRows(), numCols())
  }

  def toBlockMatrixFromCoordinate(blksByRow: Int, blksByCol: Int): Blocked = {
    ...
    new Blocked(blocks, numRows(), numCols(), newBlksByRow, newBlksByCol)
  }
   ...
   * A transpose view of this matrix
   */
  def transpose(numBlocks: Int): Blocked = {
   ...
   */
  private[mctrix] def multiplyGramianMatrixBy(v: BDV[Double]): BDV[Double] = {
    val n = numCols().toInt
   ...
   */
  private[mctrix] def computeGramianMatrix(): Matrix = {
    ...
    val GU = rows.aggregate(new BDV[Double](new Array[Double](nt)))(
      seqOp = (U, v) => {
        DenseVectored.dspr(1.0, v._2, U.data)
        U
      }, combOp = (U1, U2) => U1 += U2)
    DenseVectored.triuToFull(n, GU.data)
  }
  ...
  def computeSVD(
                  ...
                  rCond: Double = 1e-9): (DenseVectored, BDV[Double], Matrix) = {
   ...
   * @return SingularValueDecomposition(U, s, V). U = null if computeU = false.
   */
  private[mctrix] def computeSVD(
                                  k: Int,
                                  computeU: Boolean,
                                  rCond: Double,
                                  maxIter: Int,
                                  tol: Double,
                                  mode: String): (DenseVectored, BDV[Double], Matrix) = {
    val n = numCols().toInt
   ...
   */
  def multiply(B: BDM[Double]): DenseVectored = {
    ...
    new DenseVectored(result, 0L, B.cols)
  }

}


object DenseVectored {
  /**
...
private[mctrix] object EigenValueDecomposition {
  /**
   ...
   *       function.
   */
  private[mctrix] def symmetricEigs(
