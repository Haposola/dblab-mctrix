package edu.hit.dblab.mctrix.vector

import breeze.linalg.{DenseMatrix => BDM, DenseVector => BDV}
import edu.hit.dblab.mctrix.matrix.{BlockID, Blocked}
...
class DistributedVector(
  private [mctrix] val vectors: RDD[(Int, BDV[Double])],
...
//    new BlockMatrix(blocks, numRows(), numCols(), splitLen, 1)
   ...
   */
  def multiply(other: DistributedVector, mode: String = "dist"): Either[Double, Blocked] = {
    ...
    if (columnMajor == true && other.columnMajor == false){
      if (splitNum == other.splitNum){
        ...
        val result = new Blocked(blocks, length, length, splitNum, splitNum)
        Right(result)
      }else {
